...
export const PupularCourse:React.FC<PupularCourseType>=({image,description,student,price,suggestion})=>{
    return (
        <div className="flex flex-col relative flex-shrink-0 gap-3">
            ...
            <div className="flex items-center gap-1">
              <img className="w-8 h-8 rounded-full" src="/imagehome/profile.png" alt="profilr png" />
              <p>{student}</p>
            </div>
            <div className="flex gap-8 items-center ">
              <p className="text-blue-400 text-xl">{price}</p>
              <button className="p-1 bg-slate-600 rounded-lg">{suggestion}</button>
            </div>
          </div>
    )
}